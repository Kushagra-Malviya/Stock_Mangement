...
def user_dashboard():
    ...
    balance = decimal.Decimal(str(get_wallet_balance(user_id)))  # Ensure balance is Decimal
    ...
    with st.form("add_money_form"):
        ...
        amount = st.number_input("Enter amount to add", min_value=0.0, format="%.2f", key="add_money")
        submitted = st.form_submit_button("Add Money")
        
        if submitted:
            amount_decimal = decimal.Decimal(str(amount))  # Convert float to Decimal
            if amount_decimal > 0:
    ...
    stock_price = decimal.Decimal(str(get_stock_price(company_id)))  # Ensure stock price is Decimal
    ...
    with st.form("trade_form"):
        shares_input = st.number_input(
            f"Enter number of shares to {action.lower()}", min_value=1, step=1, key=f"{action}_shares"
        )
        trade_submitted = st.form_submit_button(f"{action} Shares")
        
        if trade_submitted:
            shares_to_trade = int(shares_input)  # Convert to int explicitly
            total_value = decimal.Decimal(shares_to_trade) * stock_price  # Convert shares to Decimal
            ...
            if action == "Buy":
                if balance >= total_value:
                    ...
                    current_shares = get_user_shares(user_id, company_id)
                    if current_shares > 0:
                        update_user_shares(user_id, company_id, current_shares + shares_to_trade)
                    else:
                        add_shares_to_portfolio(user_id, company_id, shares_to_trade)
                    
                    st.success(f"Successfully bought {shares_to_trade} shares of {companies[selected_company]}.")
                    st.rerun()
                else:
                ...
                current_shares = get_user_shares(user_id, company_id)
                if current_shares >= shares_to_trade:
                    new_balance = balance + total_value
                    update_wallet_balance(user_id, new_balance)
                    
                    remove_shares_from_portfolio(user_id, company_id, shares_to_trade)
                    
                    st.success(f"Successfully sold {shares_to_trade} shares of {companies[selected_company]}.")
                    st.rerun()
                else:
                    st.error("You do not have enough shares to make the sale.")
